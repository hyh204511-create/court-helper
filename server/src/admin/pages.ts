...
function usersPage(): string {
  return layout('users', 'admin', '系统用户', `
    <header class="page-head">
      <div><h2>系统用户</h2><p>管理内部登录身份、角色与会话状态。密码只在创建或重置时输入。</p></div>
    </header>
...
function platformAccountsPage(): string {
  return layout('platform-accounts', 'admin', '平台账号', `
    <header class="page-head">
      <div><h2>平台账号</h2><p>维护扩展使用的平台账号标签与启停状态；登录和按需查看凭据统一在浏览器控制台完成。</p></div>
    </header>
...
function casesPage(role: AdminRole): string {
  return layout('cases', role, '案件台账', `
    <header class="page-head">
      <div><h2>案件台账</h2><p>显示服务端当前保留期内的结果。页面可见时自动刷新，隐藏页面时暂停。</p></div>
    </header>
    <section class="panel" aria-labelledby="case-filter-title">
      ...
      <div class="panel-body">
        <form id="case-filters" class="filters">
          ...
          <div class="field"><label for="case-status">案件状态</label><select id="case-status"><option value="">全部</option><option value="立案成功">立案成功</option><option value="强执成功">强执成功</option><option value="已驳回">已驳回</option><option value="审核中">审核中</option><option value="UNKNOWN">待人工确认</option></select></div>
          ...
          <div class="field"><label for="case-human">处理状态</label><select id="case-human"><option value="">全部</option><option value="true">待人工</option><option value="false">正常</option></select></div>
          <div class="field"><label for="case-from">立案日期从</label><input id="case-from" type="date"></div>
          <div class="field"><label for="case-to">立案日期至</label><input id="case-to" type="date"></div>
    ...
    <section class="panel" aria-labelledby="case-list-title">
      <div class="panel-head"><div><h3 id="case-list-title">案件列表</h3><p>案件状态来自服务端精确结果；处理状态单独标记是否需要人工介入。</p></div></div>
      <div class="panel-body">
        ...
        <div class="table-wrap"><table class="data-table"><thead><tr><th>案号</th><th>原告</th><th>被告</th><th>案件状态</th><th>处理状态</th><th>类型</th><th>平台账号</th><th>驳回原因</th><th>查询时间</th></tr></thead><tbody id="case-rows"></tbody></table></div>
      </div>
    </section>`);
}
...
function caseDetailPage(role: AdminRole, caseId: string): string {
  return layout('case-detail', role, '案件详情', `
    <header class="page-head">
      <div><h2>案件详情</h2><p>只读查看案件字段与授权截图，状态不会由页面推断或改写。</p></div>
...
function reportExportsPage(role: AdminRole): string {
  ...
  return layout('report-exports', role, '报表导出', `
    <header class="page-head">
      <div><h2>报表导出</h2><p>查看保留期内的报表文件，按权限鉴权下载或删除。</p></div>
    </header>
...
function browserControlPage(role: AdminRole): string {
  ...
  return layout('browser-control', role, '浏览器控制', `
    <header class="page-head"><div><h2>浏览器控制</h2><p>登录、导入、查询和导出的唯一任务入口。任务由扩展在已打开的法院标签页执行。</p></div><div class="row-actions"><a class="button secondary" href="/admin/cases">查看案件台账</a><p class="subtle">当前后台用户：<strong id="current-backoffice-user">加载中…</strong></p></div></header>
