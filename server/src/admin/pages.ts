...
function browserControlPage(role: AdminRole): string {
  ...
  return layout('browser-control', role, '浏览器控制', `
    <header class="page-head"><div><h2>浏览器控制</h2><p>登录、导入、查询和导出的唯一业务入口。任务由扩展在已打开的法院标签页执行。</p></div><p class="subtle">当前后台用户：<strong id="current-backoffice-user">加载中…</strong></p></header>
