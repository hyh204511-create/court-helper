...
export class PgWecomNotificationRepository implements WecomNotificationRepository {
  ...
  async createPending(input: Omit<WecomNotificationRecord, 'id' | 'status' | 'errorCode' | 'attemptCount' | 'createdAt' | 'updatedAt' | 'sentAt'>) { const result = await this.database.query(`INSERT INTO wecom_notifications (id, case_id, platform_account_id, result_status, screenshot_id, status) VALUES ($1,$2,$3,$4,$5,'pending') ON CONFLICT (case_id,result_status) DO NOTHING RETURNING *`, [randomUUID(), input.caseId, input.platformAccountId, input.resultStatus, input.screenshotId]); if (result.rows[0]) return { record: fromRow(result.rows[0]), created: true }; const existing = await this.database.query('SELECT * FROM wecom_notifications WHERE case_id = $1 AND result_status = $2', [input.caseId, input.resultStatus]); return { record: fromRow(existing.rows[0]), created: false }; }
