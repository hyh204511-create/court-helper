...
function localConfig(adminOrigins = 'http://127.0.0.1:3000,https://admin.example.test') {
  return loadConfig({
    PORT: '3109',
    ...
    CORS_ADMIN_ORIGINS: adminOrigins,
...
test('non-loopback HTTP admin origin keeps the Secure cookie flag', async () => {
  const app = buildApp({
    config: localConfig('http://admin.example.test'),
    dependencies: {
      database: { check: async () => true },
      objectStorage: { check: async () => true },
    },
