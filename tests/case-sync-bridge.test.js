...
test("case sync bridge treats a new query as an explicit retry of needs_human", async () => {
  const calls = [];
  let stored;
  const bridge = createCaseSyncBridge({
    ensureCoordinator: async () => ({
    ...
    outbox: {
      async enqueue(input) {
        stored = { id: "outbox-needs-human", status: "needs_human", lastErrorCode: "EVIDENCE_NOT_CLOSED", ...input };
        return stored;
      },
      async retry(id) {
        calls.push(`retry-${id}`);
        stored = { ...stored, status: "pending", lastErrorCode: null, receipt: null };
      },
      async getOutbox() { calls.push("read-result"); return stored; },
    },
  });
  ...
  assert.deepEqual(calls, ["retry-outbox-needs-human", "coordinator-retry", "read-result"]);
});
