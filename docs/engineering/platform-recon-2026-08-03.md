...
## 5. 「我的立案审判 / 查看强执」模块定位 — 未验证

- 静态包中无字面「立案审判」「强执」页面；最接近的业务模块为「我的案件」（pagesAjkj 案件空间）与「网上立案」（在线立案）。
- 用户工作流中「我的立案审判」「查看强执」应为登录后首页/案件空间的入口（可能按案件类型筛选执行案件）。**必须由用户已登录会话核对**，未核对前不写选择器、不猜。
...
| 用户区元素（当前登录账号文本） | 未验证 |
| 「我的立案审判」入口与列表行结构 | 未验证 |
| 案件详情（立案成功时间/案号/驳回时间/原因）DOM | 未验证 |
| 强执 tab（顶部「执行」/左侧「我的立案」列） | 未验证 |
