...
> 版本：0.5 ｜ 状态：已确认、待实现 ｜ 依据：脱敏测试模板解析、Phase 11 控制台唯一入口决策、账号化导出决策、用户提供的 22 列空模板
...
2. 表头校验：旧双区块模板第 1 行 12 列与立案表头完全一致；新版合并模板为 22 列 A–V（U=`业务员`、V=`助理`），并保留旧 20 列 A–T 与 21 列 A–U 上传兼容。21/22 列存在时必须精确校验追加列名，其他额外列一律拒绝。
3. 块解析：立案块 = 第 2 行起，到强执表头行前；强执表头行 = 首个「A=原告 且 E=强执状态」的行（按内容识别，不依赖固定行号）；强执块 = 其后到末尾。
4. 数据行可为空：空白模板仍是合法平台发现基线，不得因 `liRows` / `qzRows` 为 `0` 阻止命令。合并模板的非空行也不提供平台案件事实，只允许 `QUERY_ALL_EXPORT` 读取 A/B/C/U/V 做导出关联；两种模式都不把模板业务结果写入 IndexedDB。
5. `QUERY_LI` / `QUERY_QZ` 仍要求当前表块没有业务数据行，非空时返回不含业务明文的 `TEMPLATE_NOT_EMPTY`。`QUERY_ALL_EXPORT` 允许合并模板包含业务行，但这些行只提供 U/V 业务归属元数据，绝不作为法院查询、状态、时间、案号、图片或案件增删依据；平台发现和采集流程保持不变。
6. 新版合并模板中，A（原告）、B（被告）、C（账号）与 U/V 构成导出时的临时业务归属映射。U/V 首尾空白清理后各自最大 100 个字符，可独立为空；映射只在受 claim 保护的批次读取链路中传给本次导出，不写入 IndexedDB、案件表、命令 payload/result、报表元数据或日志。同一 `账号 + 原告 + 被告` 出现互相冲突的非空归属时必须停止导出并返回稳定错误，禁止猜测。
7. **DISPIMG 单元格跳过**：H/K 列的 `=_xlfn.DISPIMG(...)` 公式不解析、不读图——图片由插件查询后重新截图生成。
8. 唯一键去重：按 §2 唯一键，已存在 → 更新导入字段（不覆盖已有截图/时间，除非本次导入提供新值——默认：导入仅更新基础字段，查询结果才覆盖业务字段）。
9. 返回摘要 `{ imported, updated, skipped, reasons[] }`；后台控制台展示服务器受控批次摘要，扩展不再通过 Popup 展示。
...
4. 平台是状态、成功时间、案号、成功截图、驳回时间、驳回原因和驳回截图的唯一事实来源。查询前只原子替换**当前已登录账号且当前查询类型**的本地旧记录；替换还必须清理同一账号、该类型的旧式无 `platformAccountId` 记录，防止导出混入历史。对于稳定来源 UID 相同的记录，若本轮状态识别、补证或截图失败，必须保留上一轮已由平台确认的状态、成功时间、案号、驳回时间、驳回原因及图片，同时标记本轮待人工；本轮取得新的确定事实时才覆盖对应旧值。其他账号和另一查询类型的记录不得受影响。发现记录写入后生成新的报表，绝不回写或改动导入模板。
...
> 新版 22 列合并模板是新导出唯一权威格式；旧版双区块 12 列与 20/21 列合并模板仅保留上传兼容，不再用于生成新报表。

1. 用 ExcelJS 新建工作簿，Sheet1，按新版 22 列模板样式复刻（表头、至少 10 个预留数据行、边框、对齐、字体、行高、列宽、日期格式）。
2. 布局：第 1 行固定为 A–V 单一表头；A–D 为原告、被告、账号、密码，E–L 为立案结果，M–T 为强执结果，U 为业务员，V 为助理。按 `账号 + 原告 + 被告` 合并两类记录；仅存在一侧结果时另一侧结果列留空，禁止丢弃该行或猜测配对。
3. 单元格写入：
   ...
   - 业务归属列：生成每个业务数据行时，以临时导入映射中的精确 `导入账号 + 原告 + 被告` 匹配当前导出凭据账号与当事人，命中后把该行 U/V 原样写入导出 U/V；不修改平台采集记录。未命中时 U 可回退到既有任务级业务员兼容值，V 留空。空白预留行 U/V 保持空白。
...
- 表头 22 列一致且 U/V 分别为“业务员”“助理”；同一主体的立案/强执结果位于同一行，导入行的 U/V 在导出中按案件身份保持一致；
