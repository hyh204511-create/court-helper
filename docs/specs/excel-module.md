...
4. 数据行必须为空：22 列空白模板是 `QUERY_ALL_EXPORT` 的平台发现基线，不得因 `liRows` / `qzRows` 为 `0` 阻止命令；存在任何业务数据行时返回不含业务明文的 `TEMPLATE_NOT_EMPTY`。模板不提供平台案件事实或 U/V 联系人，不把模板内容写入 IndexedDB。
5. `QUERY_LI` / `QUERY_QZ` 同样要求当前表块没有业务数据行。所有查询类型都只使用平台发现和采集流程，不按导入行驱动查询。
6. U/V 联系人仅来自所选平台账号导入并保存的固定 `salespersonName/assistantName`，不从空白模板、平台 DOM 或案件字段推断。
...
3. 单元格写入：
   ...
   - 业务归属列：生成每个业务数据行时，把所选平台账号的固定业务员/助理分别写入 U/V；两者缺少任一项时返回 `ACCOUNT_CONTACTS_UNAVAILABLE`，不得生成报表。空白预留行 U/V 保持空白。
...
- 表头 22 列一致且 U/V 分别为“业务员”“助理”；同一主体的立案/强执结果位于同一行，所有业务数据行 U/V 与所选平台账号固定联系人一致；
