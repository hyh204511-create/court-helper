...
1. 用 ExcelJS 新建工作簿，Sheet1，按 §1 样式复刻（表头、数据/预留空白行、边框、对齐、字体、行高、列宽、日期格式）。
2. 布局：立案表头第 1 行，立案数据行随后（按导入顺序或查询时间排序，默认导入顺序），且不足 3 行时保留带格式的空白行；强执表头 = `max(立案末数据行, 4) + 5`，强执数据行随后且同样至少保留 3 行。
3. 单元格写入：
