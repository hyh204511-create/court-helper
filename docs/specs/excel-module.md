...
> 版本：0.4 ｜ 状态：已确认、待实现 ｜ 依据：脱敏测试模板解析、Phase 11 控制台唯一入口决策、账号化导出决策、用户提供的 21 列空模板
...
2. 表头校验：旧双区块模板第 1 行 12 列与立案表头完全一致；合并空模板接受新版 21 列 A–U（U=`业务员`），并保留旧 20 列 A–T 上传兼容。21 列存在时必须精确校验 U 列名，其他额外列一律拒绝；业务员单元格不作为案件或导出值来源。
...
> 2026-08-09 布局决策：新版 21 列合并模板是新导出唯一权威格式；旧版双区块 12 列仅保留导入兼容，20 列合并空模板保留上传兼容，不再用于生成新报表。

1. 用 ExcelJS 新建工作簿，Sheet1，按新版 21 列模板样式复刻（表头、至少 10 个预留数据行、边框、对齐、字体、行高、列宽、日期格式）。
2. 布局：第 1 行固定为 A–U 单一表头；A–D 为原告、被告、账号、密码，E–L 为立案结果，M–T 为强执结果，U 为业务员。按 `账号 + 原告 + 被告` 合并两类记录；仅存在一侧结果时另一侧结果列留空，禁止丢弃该行或猜测配对。
3. 单元格写入：
...
- 表头 21 列一致且 U 列为“业务员”；同一主体的立案/强执结果位于同一行；
