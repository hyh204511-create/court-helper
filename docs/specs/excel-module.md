...
> 版本：0.1 ｜ 状态：待实现 ｜ 依据：用户提供 `立案与强执查询表-模板.xlsx`（2026-08-03 解析）、需求确认
...
7. 返回摘要 `{ imported, updated, skipped, reasons[] }`，popup 展示。
...
5. 文件命名：`立案与强执查询表-<导出日期YYYY-MM-DD>.xlsx`，经 popup 下载。
