...
- 查询记录写入本地库后必须等待服务器逐项案件 ACK；三种终态还必须等待对应截图上传成功以及服务端通知台账创建/幂等确认后，才算本阶段证据闭环。worker outbox 的 `sent` 只有在持久化最小服务器收据后可信；没有收据的历史 `sent` 必须幂等重放。任何未闭环记录均返回 `EVIDENCE_NOT_CLOSED`，一键查询不得进入报表导出。
