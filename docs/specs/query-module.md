...
- 驳回截图按用户提供的法院页面样式截取：保留同一详情页原生 `.fd-com-header`（法院徽标、“案件空间”与页面案件标题）以及 `.fd-content-container` 中按页面顺序出现的「重新提交信息」（存在时）和唯一「审核结果」栏目；原生页头可见案件标题必须同时包含当前待办记录中已由列表严格绑定的双方当事人精确名称，验证只用于阻断错页，不得重绘文字。截图在审核结果栏目结束，必须排除后续「基本信息」、当事人、代理人、标的、材料等栏目。不得再拼接双方当事人信息卡片，也不得用 IndexedDB、命令 payload 或 URL 重绘页头/案件标题。
