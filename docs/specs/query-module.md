...
- `layy` 与 `ajlist` 没有案件级共享 ID；`sfBh/csfid` 是账号级字段，单独使用会命中多条，禁止作为案件唯一键。`fyid` 与 `nfydm` 属于不同编码空间，禁止直接比较；法院必须以稳定翻译文本 `fymc=cfydmTranslateText` 精确匹配。候选必须同时满足 `sfBh=csfid`、`fymc=cfydmTranslateText`、`ajlx=cywlx`、`laay=claay`。`cajmc` 允许法院已确认格式：去掉精确后缀“来源案由+一案”后，按 `与`、`诉`、中英文逗号、顿号分隔为完整当事人 token；严格等于“来源原告完整名称+来源被告完整名称+来源案由”；或仅在强执时严格等于“来源申请执行人完整名称+申请+来源被执行人完整名称+来源案由”。来源同一侧含多名当事人时，只按中英文逗号、顿号拆成非空完整名称，并要求来源双方的每一个完整名称都与标题中的一个完整 token 全等；不得使用子串、前后缀或相似度匹配。`layy.updateTime` 是来源记录更新时间，不是立案/强执成功日期，不得与 `ajlist.clarq` 绑定或借用为成功时间；成功时间和案号只从唯一 `ajlist` 候选的 `clarq/cah` 读取，零条或多条仍转人工。强执来源案由为空并规范为“暂无”时不以来源案由比较 `ajlist.claay`；标题允许与来源完整案件名称精确相等，或在 `ajlist.claay` 非空且非“暂无”时严格等于“来源申请执行人完整名称+申请+来源被执行人完整名称+该候选 `ajlist.claay`”。账号、法院、执行类型和标题必须全部精确匹配并形成唯一候选；不得接受任意标题后缀。允许分隔格式标题含额外当事人，但禁止改写“诉/与”后做标题全等。
...
- 「案件空间」必须沿用受控浏览器真实点击链路（坐标只来自当前可见、已严格绑定的 `.fd-case-space-btn`），不得用合成 `.click()` 触发可能被浏览器拦截的新标签；点击后必须通过 Service Worker 确认新详情标签已接管待办，或确认本次详情采集已经清除待办。Service Worker 必须同时监听新标签创建时在 `url` 或导航提交前的 `pendingUrl` 已携带最终详情 URL，以及既有标签后续更新为详情 URL；各事件使用相同的法院详情 URL、点击前标签快照和待办校验。每次读取尚未接管的待办前还必须查询当前标签，以相同规则补偿 Service Worker 休眠、重启或事件竞态造成的事件丢失。平台复用 URL 未变化的既有详情标签时，只允许接管点击前非活动、点击后变为活动的唯一详情标签，并必须主动通知该详情 content script 重新读取当前待办；点击前后均未激活的旧详情标签不得接管。单次未确认允许再次点击 1 次，仍未打开详情标签时返回 `CASE_SPACE_TAB_UNAVAILABLE` 并阻断无图导出；点击会话无论成功失败都必须释放。
...
- 查询记录写入本地库后必须等待服务器逐项案件 ACK；三种终态还必须等待对应截图上传成功以及服务端通知实际达到 `sent`，才算最终证据闭环。worker outbox 的历史 `sent` 收据不得跳过当前查询的幂等重放；每次查询都重新取得当前服务器 ACK，使服务器换库或证据丢失后可以自动补建。任何未闭环记录均返回 `EVIDENCE_NOT_CLOSED`，一键查询不得进入报表导出。
- 平台发现重查时，即使本地同一来源记录已经保留完整的成功日期、案号和截图，也必须在本次命令中重新创建同步事件并取得新的服务器闭环收据；完整旧证据不需要再次调用“我的案件”补证接口，但不得因“不缺 F/G”跳过 outbox。补证候选筛选与最终同步候选筛选必须分离。
- 一键任务必须收集每条记录最终同步的 `clientMutationId` 作为非敏感服务器核验索引；补证重写同一记录时以前一事件为旧证据，只提交最后一次事件。报表上传后的命令成功回写必须携带这些事件 ID，服务器按当前库中的案件、终态截图与通知台账重新核验；客户端 `evidenceClosed` 布尔值或报表内嵌图片不能单独证明闭环。
- 首轮批次必须使用运行时服务器同步桥，但“立案成功/强执成功”在 `ajlist` 补齐案号与成功日期前不得进入远端 outbox，避免未完成记录提前触发企业微信；这两类只在 F/G 与截图完整后同步最终版本。`审核中` 与证据完整的驳回记录仍在首轮同步。
