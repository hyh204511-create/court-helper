...
- 状态码字典（网上立案页 zt 字段，静态分析）：11800007-1 待审核 / -2 审核通过 / -3 审核不通过 / -4 已立案 / -5 不予立案 / -6 待补充材料 / -31 待补正 / -100 待提交(UNKNOWN) / -101 申请失效(UNKNOWN) / -255 撤回中(UNKNOWN) / -500 提交失败(UNKNOWN)。`审核通过` 仅按该精确可见文本映射为立案成功，不接受近似文本。
...
- `layy` 与 `ajlist` 没有案件级共享 ID；`sfBh/csfid` 是账号级字段，单独使用会命中多条，禁止作为案件唯一键。候选必须同时满足：`sfBh=csfid`、`fyid=nfydm`、`ajlx=cywlx`、`laay=claay`、`updateTime` 与 `clarq` 规范到同一 `YYYY-MM-DD`，并把 `cajmc` 去掉精确后缀“来源案由+一案”后，按 `与`、中英文逗号、顿号分隔为当事人 token；来源原告和被告必须分别作为完整 token 存在。允许标题含额外当事人，但禁止 `includes` 单独决定匹配、禁止改写“诉/与”后做标题全等。
