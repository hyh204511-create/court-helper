...
- 驳回截图必须形成同页身份闭环：立案详情同时包含精确标题为「原告信息」「被告信息」的当事人区，强执详情同时包含精确标题为「申请执行人信息」「被执行人信息」的当事人区；每个当事人区内必须有且仅有一个表单项，其可见值与当前待办记录中已由列表严格绑定的对应当事人全等。截图将这两个经核对的当事人表单项（保留角色标题）与最新完整审核结果区域按原顺序组合为一张证据图，禁止只截审核结果、只截重新提交时间、用本地字段绘制身份文字，或把证件号码、电话、住址等非必要字段带入图片。
- 当事人区缺失、标题不唯一、对应值未唯一命中、详情身份与待办记录不一致，或组合截图目标无法建立时，保留已确认的审核时间/意见并返回 `PARTY_EVIDENCE_INCOMPLETE + needsHuman=true`；不得回退为仅审核区截图，也不得从案件标题、URL 参数或近似文本猜测当事人。
...
- 时机：对应状态页面稳定后，由 content script 使用 `captureElement` 截取已确认的案件行，或使用组合截图截取同一详情页中已核对的双方当事人表单项与最新完整审核结果区域；**截图前先确认目标信息已渲染**（双方身份、详情时间、原因可见且归属一致）。后台自动任务不得依赖需要用户手势临时授权的 `activeTab/captureVisibleTab`，也不得为截图附加 `chrome.debugger`，避免与浏览器验收控制争用调试会话；禁止截取整页或任意非目标区域。
...
- DOM 截图在克隆页面中移除已确认不承载业务事实且无法跨域读取的装饰背景（当前为 `yja-status-bg.png`）。组合截图只允许把已核对的页面源节点克隆到临时证据容器：当事人区仅保留角色标题与唯一命中的表单项，审核区保留最新完整记录；临时容器无论成功或失败都必须移除。真实平台源 DOM 只允许添加截图定位所需的临时属性，且必须清理，不得改变其样式、业务内容，也不得移除案件文字、状态、审核时间、审核意见或其他证据内容；禁止向证据图绘制来自 IndexedDB、命令 payload 或 URL 的业务文字。截图库的可恢复资源加载日志不得登记为扩展运行错误。
