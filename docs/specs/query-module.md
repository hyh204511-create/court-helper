...
- Worker 与法院页面 content script 必须在 `QUERY_ALL_EXPORT` 执行前通过固定协议版本握手。扩展重新加载后若已打开的法院列表页仍运行旧 content script，Worker 必须刷新所选列表标签并等待同版本 PING、正确列表路由和页面控件全部就绪后再读取批次或下发命令；刷新后仍不一致时以 `CONTENT_VERSION_MISMATCH` 转人工，禁止让旧执行器继续运行并产生下游误导错误码。旧的单类型查询命令保持既有兼容路径，不得借版本自愈改变其错误语义。
- `layy` 与 `ajlist` 没有案件级共享 ID；`sfBh/csfid` 是账号级字段，单独使用会命中多条，禁止作为案件唯一键。`fyid` 与 `nfydm` 属于不同编码空间，禁止直接比较；法院必须以稳定翻译文本 `fymc=cfydmTranslateText` 精确匹配。候选必须同时满足 `sfBh=csfid`、`fymc=cfydmTranslateText`、`ajlx=cywlx`、`laay=claay`。`cajmc` 允许法院已确认格式：去掉精确后缀“来源案由+一案”后，按 `与`、`诉`、中英文逗号、顿号分隔为完整当事人 token；严格等于“来源原告完整名称+来源被告完整名称+来源案由”；或仅在强执时严格等于“来源申请执行人完整名称+申请+来源被执行人完整名称+来源案由”。双方必须分别完整匹配，禁止拆词、单独 `includes` 或模糊匹配。`layy.updateTime` 是来源记录更新时间，不是立案/强执成功日期，不得与 `ajlist.clarq` 绑定或借用为成功时间；成功时间和案号只从唯一 `ajlist` 候选的 `clarq/cah` 读取，零条或多条仍转人工。强执来源案由为空并规范为“暂无”时不比较案由，改为要求 `cajmc` 与来源完整案件名称精确相等，且账号、法院、执行类型全部精确匹配并形成唯一候选。允许分隔格式标题含额外当事人，但禁止改写“诉/与”后做标题全等。
...
- 「案件空间」必须沿用受控浏览器真实点击链路（坐标只来自当前可见、已严格绑定的 `.fd-case-space-btn`），不得用合成 `.click()` 触发可能被浏览器拦截的新标签；点击后必须通过 Service Worker 确认新详情标签已接管待办，或确认本次详情采集已经清除待办。Service Worker 必须同时监听新标签创建时在 `url` 或导航提交前的 `pendingUrl` 已携带最终详情 URL，以及既有标签后续更新为详情 URL；各事件使用相同的法院详情 URL、点击前标签快照和待办校验。每次读取尚未接管的待办前还必须查询当前标签，以相同规则补偿 Service Worker 休眠、重启或事件竞态造成的事件丢失；点击前已存在且 URL 未变化的旧详情标签不得被补偿接管。单次未确认允许再次点击 1 次，仍未打开详情标签时返回 `CASE_SPACE_TAB_UNAVAILABLE` 并阻断无图导出；点击会话无论成功失败都必须释放。
- 详情 content script 在 `document_start` 注入时不得假设 SPA 的详情 hash 已经建立：首次分发未进入详情路由时必须监听后续 `hashchange`，第一次进入案件空间详情路由后执行详情待办接管；同一页面同时只允许一个详情采集任务，重复 hash 事件不得并发或重复采集。Service Worker 校验消息来源时必须使用首个非空的 `sender.tab.url` 或 `sender.url`；创建早期 `sender.tab.url` 为空不得遮蔽合法的法院来源 URL，非 `zxfw.court.gov.cn` 来源仍须返回 `UNTRUSTED_SENDER`。
