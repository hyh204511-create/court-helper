...
- `QUERY_LI` 从网上立案页完成发现与状态采集后，可在同一已登录页面会话中进入「我的案件」页，并对每一个待补全成功案件把**完整的** `sourceCaseName` 作为平台搜索条件。搜索结果必须与来源记录的原告、被告、案由身份一致；当前页面只提供组合案件标题而不提供独立参与人/案由字段时，以 `caseName` 与 `sourceCaseName` 修剪后全等作为该三项身份一致的严格替代，不拆词、不包含匹配。存在多条相同身份结果时，只接受 `立案日期` 唯一最新且案号完整的一条写入 F/G；成功截图仍取网上立案页已确认的成功列表行。
