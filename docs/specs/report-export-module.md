...
> 版本：1.1 ｜ 状态：已确认、待实现 ｜ 依据：Phase 11 后台控制台唯一入口决策、browser-command-module、server-module（存储/鉴权/错误模型复用）、用户提供的 22 列空模板
...
> v1.0 变更：新生成报表固定为 22 列，U/V 从导入批次逐案件业务归属映射写入；映射不修改法院平台采集数据或案件存储。
> v1.1 变更：企业微信通知发送结果不再作为报表生成的证据闭环条件；案件同步和终态截图已入库时允许导出，通知失败仍单独待人工重试。
> v1.2 变更：一键导出不再手填业务员，也不从导入报表业务行映射 U/V；控制台只选择平台账号和 22 列空白模板，U/V 统一取所选平台账号导入并保存的固定业务员/助理。
...
- 导出工作簿固定为 A–V 单表头：A–D 主体与账号，E–L 立案结果，M–T 强执结果，U/V 为所选平台账号固定的业务员/助理；按 `账号 + 原告 + 被告` 将两类查询结果合并到同一行，单侧缺失时保留空白结果列。
...
- `QUERY_ALL_EXPORT` 绑定的导入批次只验证并提供 22 列空白模板基线，不提供 U/V 业务归属。SW 从所选平台账号的受控凭据出口临时取得固定业务员/助理并统一写入所有导出业务行 U/V；缺少任一联系人返回 `ACCOUNT_CONTACTS_UNAVAILABLE`。不把联系人加入平台采集记录、IndexedDB、命令 payload/result、上传接口或报表元数据。
- 后台“导出成功”以服务端创建记录或幂等命中为准；仅本地下载、未配置或上传失败不显示为后台成功记录。
- `QUERY_ALL_EXPORT` 只有在每一类都完成安全处理后才进入导出：某类有记录时必须通过结构/会话/账号/选择器/API-DOM 校验；某类没有记录时必须取得结构化 `total=0` 且当前 DOM 无该类行的确认空结果。只要至少一类产生可导出记录，即可生成仅含该类数据的报表；确认为空的另一类不阻断。任一无法确认空结果或其他硬失败不得读取旧数据生成混合报表。
- **证据完整性闸门**：任一案件为 `UNKNOWN`、`needsHuman=true`，或立案/强执成功记录缺少成功时间、案号、成功图片，或已驳回记录缺少驳回时间、驳回原因、驳回图片时，`QUERY_ALL_EXPORT` 必须停止且不得生成、下载或上传工作簿；返回批次首个稳定错误码（无具体码时 `NEEDS_HUMAN`）。禁止继续产出“状态存在但图片、时间或案号为空”的残缺报表。普通 `审核中` 且无需图片/案号的完整记录不受此闸门影响。
- 企业微信通知配置、排队或发送结果不属于报表证据完整性闸门；终态截图已入库即可继续导出。通知失败仍按企业微信模块保留失败台账和人工重试能力。
...
- 导出前按当前页面账号和命令绑定的 `platformAccountId` 查询两张本地案件表；两表合计为 0 行时返回稳定错误 `REPORT_EMPTY`，不得创建 Blob、触发本地下载、上传服务器或回写 `SUCCESS`。非空记录必须通过前述证据完整性闸门；`UNKNOWN`、`needsHuman` 或缺少必需证据时停止导出，不得猜测补齐。
