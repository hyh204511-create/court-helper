...
- 导出工作簿固定为 A–V 单表头：A–D 主体与账号，E–L 立案结果，M–T 强执结果，U/V 为导入案件对应的业务员/助理；按 `账号 + 原告 + 被告` 将两类查询结果合并到同一行，单侧缺失时保留空白结果列。
...
- `QUERY_ALL_EXPORT` 从受 claim 保护的导入批次读取最小业务归属映射，并只在构建工作簿时按真实导出凭据账号、原告、被告精确匹配后写入 U/V；不把这两个字段加入平台采集记录、IndexedDB、上传接口或报表元数据。既有 `payload.salesperson` 仅作为旧模板未命中映射时的 U 列兼容回退，不能覆盖已匹配的导入值；V 未命中时留空。
