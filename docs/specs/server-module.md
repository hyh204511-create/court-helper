...
- `clientUid` 为客户端不透明标识。服务器可为 PostgreSQL 持久化单独转义其中的 NUL 字符，但 `accepted[]` / `conflicts[]` 必须回显本次请求的原始 `clientUid`，不得用数据库转义值破坏逐项 ACK 匹配；幂等查找和落库仍使用转义后的数据库标识。
- outbox 把事件标记为 `sent` 时必须同时持久化最小服务器收据：案件逐项 ACK 固定为 `caseAccepted=true`；终态事件还必须在截图上传接口成功返回（该接口已按企业微信规格确认通知台账创建或幂等存在）后记录 `screenshotStored=true`。content → worker 桥只接受同时满足当前事件类型的完整收据。升级前遗留或异常产生的 `sent` 事件若没有完整收据，不得直接当作成功；必须按相同 `clientMutationId` 重新置为 `pending` 并走服务端幂等重放，仍未闭环则返回 `EVIDENCE_NOT_CLOSED`，禁止继续导出或回写任务成功。
